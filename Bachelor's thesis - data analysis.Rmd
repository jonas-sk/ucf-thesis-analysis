...
ggsave(plot = plot.summary.violations, 
       "figure-summary-violations-obligations.png", 
...
```{r}
cases.viol.tree$plot(filename = "cases-viol-tree-plot.pdf")
cases.force.tree$plot(filename = "cases-force-tree-plot.pdf")
cases.op.tree$plot(filename = "cases-op-tree-plot.pdf")
cases.law.tree$plot(filename = "cases-law-tree-plot.pdf")
```
...
data.frame("sensitivity_ci" = binom.test(53, 61)$conf.int, #Couldn't find way to extract values
           ...
           "bacc_ci" = binom.test(round(cases.viol.tree$error.test.repeats$`Balanced Accuracy`* 77), 77)$conf.int)  %>% 
  #Alternative means of calculating BACC CI by using normal distribution (but upper bound >1)
  mutate(lower_ci = row_number(sensitivity_ci) %% 2 == 1,
         bacc_err = 1.96 *
           ...
           / sqrt(10),
         bacc_ci_normal = ifelse((lower_ci == 1), #Use normal instead of binomial distribution
           cases.viol.tree$error.test.repeats$`Balanced Accuracy` - bacc_err,
           cases.viol.tree$error.test.repeats$`Balanced Accuracy` + bacc_err)) %>% 
  select(-bacc_err)
```
...
data.frame("sensitivity_ci" = binom.test(28, 38)$conf.int, #Couldn't find way to extract values
           ...
           "bacc_ci" = binom.test(round(cases.force.tree$error.test.repeats$`Balanced Accuracy`* 77), 77)$conf.int)
```
...
data.frame("sensitivity_ci" = binom.test(34, 43)$conf.int, #Couldn't find way to extract values
           ...
           "bacc_ci" = binom.test(round(cases.op.tree$error.test.repeats$`Balanced Accuracy`* 77), 77)$conf.int)
```
...
cases.viol.tree.full.viz %>% 
  ...
  rsvg_svg(file = "cases-viol-tree-generated.svg")
```
...
cases.force.tree.full.viz %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-force-tree-generated.svg")
```
...
cases.op.tree.full.viz %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-op-tree-generated.svg")
```
...
cases.law.tree.full.viz %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-law-tree-generated.svg")
```
...
```{r}
grViz("cases-viol-tree-graph.gv") %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-viol-tree-graph.svg")
```

```{r}
grViz("cases-force-tree-graph.gv") %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-force-tree-graph.svg")
```

```{r}
grViz("cases-op-tree-graph.gv") %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-op-tree-graph.svg")
```

```{r}
grViz("cases-law-tree-graph.gv") %>% 
  export_svg() %>% charToRaw() %>% 
  rsvg_svg(file = "cases-law-tree-graph.svg")
```
